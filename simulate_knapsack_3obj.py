...
for r_obj in [0.1]:
    for r_con in [0.4,0.5,0.6,0.7,0.8]:
        print(f"file = {n_obj}, r_obj = {r_obj}, r_con = {r_con}")
        r = -r_obj * np.ones((n_obj + n_con, n_obj + n_con)) + (1 + r_obj) * np.eye(n_obj + n_con)  #right now not dealing with different r for obj and con
        ...
        output_dir = "./results/runA"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        olab = int(10*r_obj)
        clab = int(10*r_con)
        savemat(os.path.join(output_dir, f"kn_{olab}_{clab}_{type}_{n_items}_{n_selected}_{n_obj}.mat"), allsim, store_python_metadata=True)

        file_path = os.path.join(output_dir, f"kn_{olab}_{clab}_{type}_{n_items}_{n_selected}_{n_obj}.pkl")
