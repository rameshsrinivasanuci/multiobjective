...
def sample_population_conditional(
    ...
    while pop_count < pop_size:
        ...
        # ------ all objectives reweighting ------
        reweights = np.array([10.0, 1.0, 1.0]).astype(float)
        Xn = (samples - samples.min(axis=0)) / (samples.ptp(axis=0) + 1e-12)
        v = Xn[:, :len(reweights)] @ reweights
        ...
        for n in range(1, n_selected):
            ...
            # ------ all objectives reweighting ------
            reweights = np.array([10.0, 1.0, 1.0]).astype(float)
            Xn = (x_candidates - x_candidates.min(axis=0)) / (x_candidates.ptp(axis=0) + 1e-12)
            v = Xn[:, :len(reweights)] @ reweights
            r = v.argsort().argsort().astype(float)
            s = r / (r.max() + 1e-12)
            logits = s / 0.03
...
def main():
    ...
    for i in range(3):
        ...
        results = eda.run()
        file = f'softmax_blendver_mode1_2_3_{n_items}_{n_selected}_obj{n_obj}_run{i}.pkl'
